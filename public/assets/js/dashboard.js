...
document.addEventListener("DOMContentLoaded", () => {
  // Sidebar nav
  document
    .querySelectorAll(".dash-sidebar .nav-item[data-page]")
    .forEach((el) => {
...
function switchPage(page) {
  ...
  document
    .querySelectorAll(".dash-sidebar .nav-item")
    .forEach((n) => n.classList.remove("active"));
  const active = document.querySelector(
    `.dash-sidebar .nav-item[data-page="${page}"]`,
  );
...
async function loadOverview() {
  try {
    ...
    const sorted = stats
      .filter((s) => parseInt(s.total_ratings) > 0)
      .sort((a, b) => {
        const avgA = parseFloat(a.avg_value);
        const avgB = parseFloat(b.avg_value);
        if (avgB !== avgA) {
          return avgB - avgA;
        }
        return parseInt(b.total_ratings) - parseInt(a.total_ratings);
      })
      .slice(0, 5);
    if (!sorted.length) {
      topEl.innerHTML =
        '<div class="text-muted small text-center py-3">Belum ada penilaian.</div>';
    } else {
...
function renderEmployees() {
  ...
  document.getElementById("emp-page-info").textContent = total
    ? `${(empPage - 1) * PER_PAGE + 1}–${Math.min(empPage * PER_PAGE, total)} dari ${total}`
    : "";
  ...
  for (let p = 1; p <= pages; p++) {
    ul.insertAdjacentHTML(
      "beforeend",
      `<li class="page-item ${p === empPage ? "active" : ""}">
...
async function deleteEmployee(id, name) {
  const { isConfirmed } = await Swal.fire({
    title: `Hapus ${name}?`,
    text: "Data pegawai dan ratingnya akan terhapus.",
    icon: "warning",
...
function renderRatingEmployeeStats() {
  ...
  el.innerHTML = ratingStats
    .map((emp) => {
      ...
      const sc =
        avg >= 4 ? "text-success" : avg >= 2.5 ? "text-primary" : "text-danger";
...
function renderRatingTable() {
  ...
  document.getElementById("rat-page-info").textContent = total
    ? `${(ratPage - 1) * PER_PAGE + 1}–${Math.min(ratPage * PER_PAGE, total)} dari ${total}`
    : "";
  ...
  for (let p = 1; p <= pages; p++) {
    ul.insertAdjacentHTML(
      "beforeend",
      `<li class="page-item ${p === ratPage ? "active" : ""}">
...
async function ratDelete(id) {
  ...
  if (!isConfirmed) return;
  try {
    const res = await fetch("/api/rating/delete", {
      method: "DELETE",
    ...
    Swal.fire({ icon: "error", title: "Error", text: "Kesalahan jaringan." });
  }
}
...
async function loadSKM() {
  ...
  try {
    ...
    if (!data.length) {
      tbody.innerHTML =
        '<tr><td colspan="16" class="text-center py-4 text-muted">Tidak ada data.</td></tr>';
    } else {
      tbody.innerHTML = data
        .map((d) => {
          let qs = "";
          for (let i = 1; i <= 9; i++)
            qs += `<td class="text-center">${d["quest_" + i] ?? ""}</td>`;
          return `<tr>
...
async function loadZI() {
  ...
  try {
    ...
    if (!data.length) {
      tbody.innerHTML =
        '<tr><td colspan="22" class="text-center py-4 text-muted">Tidak ada data.</td></tr>';
    } else {
      tbody.innerHTML = data
        .map((d) => {
          let qs = "";
...
function jobBadge(job) {
  ...
  return (
    m[job] ??
    '<span class="badge badge-job bg-secondary-subtle text-secondary">—</span>'
  );
}
